        ...
        try {
            ...
            if(categoryOffers){
                for (const offer of categoryOffers) {
                    ...
                    const discountPercentage = offer.offerPrice;
               
                    const category = await Category.find({Name: categoryName})

                    const discountMultiplier = 1 - discountPercentage / 100;
               
                  await Product.updateMany({category: category[0]._id},{$mul:{discountPrice: discountMultiplier  }})
                }
            }
            await Product.updateMany({}, { $set: { discountPrice: { $round: ['$discountPrice', 2] } } });
